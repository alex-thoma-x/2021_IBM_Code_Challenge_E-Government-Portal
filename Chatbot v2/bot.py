...
while i is True:
    ...
    result = random.choice(p['responses'])
    link=p['context']
    import webbrowser
    if(link[0] != ''):
       webbrowser.open(link[0], new=0, autoraise=True)
       i=False

    translator = Translator()
    result = translator.translate(result, dest='ml', src='en')
    print(result.text)
    ...
    s=input("Press X to continue and E to stop")
    if s=='x':
        i=True
    elif s=='e':
        i=False
